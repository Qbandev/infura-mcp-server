...
export function formatAsMarkdown(result, toolName) {
  ...
  switch (toolName) {
    ...
    case 'eth_getLogs': {
      // eth_getLogs may return either a flat array of logs or a paginated
      // object with { logs, pagination }. Normalize for formatLogs.
      const logsArray = Array.isArray(result)
        ? result
        : (result && Array.isArray(result.logs) ? result.logs : result);
      return formatLogs(logsArray, toolName);
    }
