 ...
 * not all data is shown due to character limits. It includes the item count
 * for context and suggests using pagination or filters for complete results.
 *
 * @function truncationMessage
 * @param {number} itemCount - The number of items in the result
 * @param {number} charLimit - The character limit where truncation occurred
 * @returns {string} A formatted truncation notice message
 *
 * @example
 * // Response exceeded CHARACTER_LIMIT
 * if (textContent.length > CHARACTER_LIMIT) {
 *   const truncated = textContent.substring(0, CHARACTER_LIMIT);
 *   const message = truncationMessage(logs.length, CHARACTER_LIMIT);
 *   return truncated + message;
 * }
 * // Output: "\n\n[Response truncated at 100,000 characters. 5000 items found. Use pagination or filters for complete results.]"
 *
 * @example
 * truncationMessage(5000, 100000);
 * // Returns: "\n\n[Response truncated at 100,000 characters. 5000 items found. Use pagination or filters for complete results.]"
 */
export const truncationMessage = (itemCount, charLimit) => {
  const itemText = itemCount === 1 ? "1 item" : `${itemCount} items`;
  return `\n\n[Response truncated at ${charLimit.toLocaleString()} characters. ${itemText} found. Use pagination or filters for complete results.]`;
};
